...
/* ----------------------------
   Apps Data (typed, safe)
----------------------------- */
type AppItem = {
  title: string;
  description: string;
  technologies: string[];
  codeLink?: string; // optional to avoid dead links
  image: ImageKey;
  alt: string;
};
...
const apps = [
  ...
  },
  {
    ...
    // no codeLink yet → button won't render
    image: "twitterClone",
  ...
  },
  {
    ...
    image: "certificate", // ✅ maps to /public/images/certificate.jpg via ImageKey
  ...
  return (
    ...
    >
      <Container className="text-center">
        ...
        >
          {apps.map((app, index) => (
            <motion.li key={app.title} variants={fadeUp(index + 2)} className="h-full">
              {/* ProjectCard already renders a <Card> root with interactive polish */}
              <ProjectCard {...app} />
            </motion.li>
          ))}
        </motion.ul>
